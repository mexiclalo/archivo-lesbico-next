...
export default function Header() {
  ...
  const labels = {
    es: { chronology: 'CRONOLOGÍA', events: 'ÍNDICE CRONOLÓGICO DE EVENTOS' },
    en: { chronology: 'CHRONOLOGY', events: 'CHRONOLOGICAL EVENTS INDEX' }
  };
                              ...
                              <div key={year.name} className="relative group/year">
                                ...
                                {year.items && (
                                  <div className="absolute top-0 left-full ml-[1px] w-64 bg-[#291147]/99 backdrop-blur-xl border border-white/10 opacity-0 invisible group-hover/year:opacity-100 group-hover/year:visible transition-all duration-300 shadow-2xl rounded-sm">
                                    {year.items.map((item) => (
                                      <Link key={item.name} href={item.href} className="block px-6 py-4 text-white hover:bg-[#8C0DC2] transition-colors text-[9px] uppercase tracking-widest leading-tight">
                                        {item.name}
                                      </Link>
                                    ))}
                                  </div>
            ...
            <div key={item.name}>
              {item.submenu ? (
                <div className="space-y-4">
                  <button onClick={() => setIsCronologiaOpen(!isCronologiaOpen)} className="w-full text-left text-white text-lg font-bold tracking-widest flex justify-between items-center">
                        ...
                        <div className={`${openDecade === sub.name ? 'max-h-[1000px] opacity-100 pb-2' : 'max-h-0 opacity-0'} overflow-hidden transition-all duration-300 pl-4 space-y-4`}>
                          ...
                          {sub.years.map((year) => (
                            <div key={year.name} className="space-y-2">
                              ...
                              {year.items && year.items.map(subItem => (
                                <Link key={subItem.name} href={subItem.href} onClick={closeMenu} className="block text-white/50 text-xs pl-4 uppercase tracking-tighter">
                                  — {subItem.name}
                                </Link>
                              ))}
                            </div>
